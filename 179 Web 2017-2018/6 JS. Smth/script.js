...
function check_blocks(pole, new_block) {
	blocks = pole.getElementsByClassName('block');
	// console.log(blocks);
	for (var i = 0; i < blocks.length; i++) {
		// console.log(blocks[i]);
		if (new_block.style.backgroundColor == blocks[i].style.backgroundColor) or (new_block.style.left == blocks[i].style.left) of (new_block.style.top == blocks[i].style.top) {
			new_block.remove();
		}
	}
}
